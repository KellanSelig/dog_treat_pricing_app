...
from grpc_service.v1 import service_pb2 as pb2
...
def convert_pb2_get_dog_treat_price_request_to_core(request: pb2.GetDogTreatPriceRequest) -> PricingRequest:
    ...
    return PricingRequest(
        ...
        tricks_to_perform=[
            DogTrick(pb2.DogTrick.Name(t).replace("DOG_TRICK_", "").lower()) for t in request.tricks_to_perform
        ],
        behaviors_to_perform=[
            DogBehavior(pb2.DogBehavior.Name(b).replace("DOG_BEHAVIOR_", "").lower()) for b in request.cutes_to_include
        ],
        location=Location(
            pb2.Location.Name(request.location).replace("LOCATION_", "").lower()
            if request.location
