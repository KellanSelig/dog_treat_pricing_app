...
from grpc_service.v1.service_pb2 import GetDogTreatPriceResponse
from grpc_service.v1.service_pb2 import HealthCheckResponse
from grpc_service.v1.service_pb2 import PerformDogTrickRequest
from grpc_service.v1.service_pb2 import PerformDogTrickResponse
from grpc_service.v1.service_pb2 import Status
from grpc_service.v1.service_pb2_grpc import DogTrickServiceServicer
...
class DogTrickServicer(DogTrickServiceServicer):
    ...
    def GetDogTreatPrice(
        ...
        try:
            ...
            pricing_result = self.core_service.price_dog_tricks(
                convert_pb2_get_dog_treat_price_request_to_core(request)
            )
            return convert_core_get_dog_treat_price_response_to_pb2(pricing_result, request)
        except Exception as error:
            log.exception("Error while processing GetDogTreatPrice")
            return GetDogTreatPriceResponse(status=Status.STATUS_ERROR, error=str(error))
    ...
    @override
    def HealthCheck(self, request: empty_pb2.Empty, context: grpc.ServicerContext) -> HealthCheckResponse:
